...
export function allowed(role:Role,path:string){if(/^\/operations(?:\/|$)/.test(path))return role==='SUPER_ADMIN';return isAdministrator(role)||['/','/approvals','/review','/published'].includes(path)||/^\/(news|posts|media)\/[^/]+$/.test(path);}
