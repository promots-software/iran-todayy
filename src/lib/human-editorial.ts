...
export async function saveHumanDraft(db:PrismaClient,input:EditorialTarget & {revision:number;title:string;body:string},actor:string){
 ...
 return db.$transaction(async tx=>{
  ...
  if(current?.title===text.title&&current.body===text.body&&(input.revision===current.revision||input.revision===current.revision-1))return current;
  ...
  const draft=current?await tx.humanEditorialDraft.update({where:{id:current.id},data:{...text,revision:{increment:1},status:'DRAFT',editedBy:actor,approvedAt:null,approvedBy:null,approvalNote:null}}):await tx.humanEditorialDraft.create({data:{...source.where,...text,originalSnapshot:json(source.snapshot),editedBy:actor}});
...
export async function approveHumanDraft(db:PrismaClient,input:{id:string;digest:string;confirmed:boolean;note:string},actor:string,env:Record<string,string|undefined>=process.env){
 if(!actor.trim())throw new ProcessingError('AUTHENTICATION_REQUIRED');
 ...
 const {chatId}=readPublisherEnv(env);
 return db.$transaction(async tx=>{
  await lockEditorialPublication(tx);assertApprovalMode((await tx.appSettings.findUniqueOrThrow({where:{id:1}})).publishingMode);
  ...
  if(existing){if(existing.idempotencyKey===input.digest)return existing;throw new ProcessingError('PUBLICATION_LOCKED');}
  await origin(tx,{kind:d.newsItemId?'news':'post',id:d.newsItemId??d.sourcePostId!});
  const p=await tx.publication.create({data:{humanDraftId:d.id,idempotencyKey:humanDigest(d),destination:chatId,contentSnapshot:humanText(d)}});
