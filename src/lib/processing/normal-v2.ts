...
export async function normalStage<T>(stage:string,run:(repair?:StageRepair)=>Promise<T>):Promise<T>{
 try{return await run();}catch(error){
  ...
  const repair:StageRepair={stage,code:error.code,issues:error.diagnostic&&'issues' in error.diagnostic?error.diagnostic.issues:[],instructions:'One final bounded repair. Correct only this stage using the original source and existing validated evidence. Never invent or randomly assign evidence IDs. Every sentence needs supporting existing factIds; source-unit IDs are not factIds. Remove or faithfully rephrase unsupported wording without dropping material facts. All original schema, evidence, attribution, coverage and semantic validators run again. Do not reconsider already completed relevance/content selection.'};
  try{return await run(repair);}catch(remaining){
   if(remaining instanceof ProcessingError&&repairable.has(remaining.code))throw new ProcessingError('AI_SCHEMA_REPAIR_FAILED',false,{stage,causeCode:remaining.code,issues:remaining.diagnostic&&'issues' in remaining.diagnostic?remaining.diagnostic.issues:[]});
   throw remaining;
  }
 }
}
