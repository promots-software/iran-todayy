...
export const arabicReasons:Record<string,string>={
 REVIEW_RECEIPT_INVALID:'تقرير التحقق غير متسق؛ لم يثبت خطأ في نص الخبر',
 APPLICATION_CONTINUATION_BUDGET:'بلغت المحاولة حد الطلبات المحلي؛ تستأنف من المراحل المحفوظة',
...
export function isTechnicalFailure(code:string){
 if(code==='PROPOSITION_RECEIPT_INVALID'||code==='APPLICATION_CONTINUATION_BUDGET'||code==='REVIEW_RECEIPT_INVALID'||code==='AI_CANONICAL_RENDER_REQUIRED'||isProviderWait(code)||code==='SOURCE_PROCESSING_MODE_CHANGED'||code==='SOURCE_DISABLED')return true;
