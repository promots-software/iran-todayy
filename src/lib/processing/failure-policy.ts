...
export const providerWaitCodes=['PROVIDER_BUDGET_EXHAUSTED','PROVIDER_COOLDOWN','PROVIDER_CAPACITY_WAIT','PROVIDER_RPM_WAIT','PROVIDER_TPM_WAIT','PROVIDER_RPD_WAIT','PROVIDER_COST_WAIT','PROVIDER_HOURLY_WAIT','GEMINI_HTTP_429'];
export function isProviderWait(code:string){return providerWaitCodes.includes(code);}
const budget=/^(?:PROVIDER_REQUEST_LIMIT|GROQ_REQUEST_LIMIT|PROVIDER_BUDGET_EXHAUSTED|PROVIDER_COOLDOWN|PROVIDER_(?:CAPACITY|RPM|TPM|RPD|COST|HOURLY)_WAIT|MATCH_SNAPSHOT_CHANGED)$/;
export function failurePolicy(code:string,attempt:number,retryAfterMs=0){
 ...
 return {retryable,providerFailure,
  ...
  safeCheckpointRetry:/^(?:GEMINI_HTTP_(?:429|5\d\d)|GROQ_(?:RATE_LIMIT|UNAVAILABLE)|PROVIDER_(?:REQUEST_LIMIT|BUDGET_EXHAUSTED|COOLDOWN|(?:CAPACITY|RPM|TPM|RPD|COST|HOURLY)_WAIT)|GROQ_REQUEST_LIMIT)$/.test(code),
 };
}
