...
export async function publishWeb(db:PrismaClient,input:{id:string;digest:string;confirmed:boolean},actor:string){
 ...
 return db.$transaction(async tx=>{
  ...
  if(p.humanDraft){const d=p.humanDraft;if(d.status!=='APPROVED'||!d.approvedAt||!d.approvedBy||!d.approvalNote||!matchesHumanPublication(d,p)||humanText(d)!==p.contentSnapshot||(d.publicationImageId??null)!==(p.publicationImageId??null))throw new Error('APPROVAL_CHANGED');}
