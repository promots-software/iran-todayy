...
type UpdateItem={eventRevisionId:string;factualEvidence:unknown;validationResult:unknown;eventRevision:{facts:unknown};evidence:{sourcePost:{id:string;originalContent:string;normalizedContent?:string|null;processingResult:unknown}}[]};
...
export function normalMaterialUpdateFacts(item:UpdateItem){
 ...
 const extraction=validateUnderstanding(receipt.extraction,processingSource(post));
