...
export default async function NewsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><PageTitle title={item.title} description={`سجل الخبر · ${item.id}`} /><div className="section-title"><Badge value={item.status} /><span className="muted">{date(item.createdAt)}</span></div>
    ...
    {!item.humanDraft&&!item.publication&&!item.error&&['PASSED','NEEDS_REVIEW'].includes(item.validationStatus)&&['NEEDS_REVIEW','PENDING_APPROVAL'].includes(item.status)&&<PublicationApproval id={item.id} digest={approvalDigest(item)} reviews={[...new Map(review.map(r=>[reviewKey(r),{key:reviewKey(r),label:`${r.explanation??r.code}${r.detail?` — ${r.detail}`:''}`}])).values()]}/>}
