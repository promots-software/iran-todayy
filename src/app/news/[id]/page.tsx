...
import {reviewMessages,technicalExplanation} from '@/lib/processing/editorial-eligibility';
...
export default async function NewsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await readDatabase(() => db.newsItem.findUnique({ where: { id }, include: {...newsInclude,humanDraft:true} }));
  ...
  return <><PageTitle title={item.title} description={`سجل الخبر · ${item.id}`} /><div className="section-title"><EditorialState value={item.validationResult} status={item.status} error={item.error}/><span className="muted">{date(item.createdAt)}</span></div>
    <section className="panel"><h2>الصياغة العربية</h2><p className="original">{item.arabicContent ?? "لم تُنتج صياغة بعد"}</p><dl className="facts"><dt>التحقق</dt><dd><Badge value={item.validationStatus} /></dd><dt>وضع النشر عند المعالجة</dt><dd>{label(item.modeAtProcessing)}</dd><dt>أسباب المراجعة</dt><dd>{reviewMessages(review).join(" / ") || "—"}</dd><dt>سبب الرفض</dt><dd>{item.rejectionReason ?? "—"}</dd><dt>الاعتماد</dt><dd>{date(item.approvedAt)} {item.approvedBy}</dd><dt>بداية / نهاية المعالجة</dt><dd>{date(item.processingStartedAt)} / {date(item.processingEndedAt)}</dd><dt>خطأ المعالجة</dt><dd>{item.error ? technicalExplanation : "—"}</dd><dt>نسخة القواعد</dt><dd>{item.ruleSet?.version ?? "لم تُحدد"}</dd></dl><details><summary>التحقق والأدلة والاقتباسات المحمية</summary><JsonView value={{ validation: item.validationResult, facts: item.factualEvidence, protectedQuotes: item.protectedQuotes }} /></details></section>
    {!item.humanDraft&&!item.publication&&!item.error&&['PASSED','NEEDS_REVIEW'].includes(item.validationStatus)&&['NEEDS_REVIEW','PENDING_APPROVAL'].includes(item.status)&&<PublicationApproval id={item.id} digest={approvalDigest(item)} reviews={[...new Map(review.map(r=>[reviewKey(r),{key:reviewKey(r),label:reviewMessages([r])[0]??'إقرار مراجعة الاختبار قبل الإرسال اليدوي'}])).values()]}/>}
    <p>{item.humanDraft?'يوجد تحرير بشري مستقل؛ حالة التحقق الآلي الأصلية لم تتغير.':item.validationStatus==='PASSED'?'AI validated — اجتاز التحقق الآلي':'التحقق الآلي يحتاج مراجعة أو لم ينجح'}</p>
    {!item.publication&&['NEEDS_REVIEW','PENDING_APPROVAL','FAILED','REJECTED'].includes(item.status)&&<HumanEditorialPanel kind="news" id={item.id} initialTitle={item.title} initialBody={item.arabicContent??''}/>}
