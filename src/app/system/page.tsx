...
export default async function SystemPage() {
 await requireUser(true);
  ...
  const duration=(ms:number|null)=>ms===null?'غير مقاس':`${Math.round(ms/1000)} ثانية`;
  return <><PageTitle title="حالة النظام" description="اتصال قاعدة البيانات وآخر نبضة للعامل وحالة مهام المعالجة." />{!result.available && <DatabaseNotice />}
    {result.available&&<section className="panel"><h2>صحة طابور الأخبار</h2><dl className="facts"><dt>بانتظار المعالجة</dt><dd>{result.data.queue.counts.PENDING??0}</dd><dt>عمر أقدم خبر منتظر</dt><dd>{duration(result.data.queue.oldestPendingMs)}</dd><dt>قيد المعالجة</dt><dd>{result.data.queue.counts.RUNNING??0}</dd><dt>إعادة المحاولة</dt><dd>{result.data.queue.counts.RETRY??0}</dd><dt>المهام المنتهية خلال ساعة</dt><dd>{result.data.queue.finishedLastHour}</dd><dt>انتظار الطابور — الوسيط / P95</dt><dd>{duration(result.data.queue.queueWait.medianMs)} / {duration(result.data.queue.queueWait.p95Ms)}</dd><dt>المعالجة — الوسيط / P95</dt><dd>{duration(result.data.queue.processing.medianMs)} / {duration(result.data.queue.processing.p95Ms)}</dd><dt>توقف المزود المؤقت حتى</dt><dd>{result.data.queue.cooldownUntil?date(result.data.queue.cooldownUntil):'لا يوجد'}</dd><dt>القبول التالي وفق الميزانية</dt><dd>{date(result.data.queue.nextAdmission)}</dd><dt>المحاولة المجدولة التالية</dt><dd>{result.data.queue.nextRetry?date(result.data.queue.nextRetry):'لا توجد'}</dd></dl></section>}
