...
export function checkpointProvider(provider:LanguageProvider, store:CheckpointStore):LanguageProvider {
  async function stage<T>(name:string,input:unknown,signal:AbortSignal,call:()=>Promise<T>):Promise<T> {
    ...
    const key=createHash('sha256').update(JSON.stringify([directBilingual?'worker-direct-bilingual-v2':name==='understand'&&context.processingMode==='DIRECT'?'worker-direct-arabic-publication-v1':name==='draft'?'worker-local-draft-v3':'worker-checkpoint-v1',provider.id,name,input])).digest('hex');
