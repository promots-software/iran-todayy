...
import {allowed,type Role} from '@/lib/dashboard-permissions';
...
const routes=[['/','نظرة عامة'],['/sources','المصادر'],['/approvals','الموافقات'],['/review','المراجعات'],['/published','الأخبار المنشورة'],['/filtered','المستبعد والمرفوض'],['/events','الأخبار المكررة'],['/logs','سجل العمليات'],['/settings','الإعدادات'],['/system','حالة النظام']];
export function DashboardShell({user,children}:{user:{displayName:string;role:Role}|null;children:React.ReactNode}){
 ...
 <div className="workspace"><header className="topbar"><button ref={menuButton} className="mobile-only secondary" aria-controls="navigation" aria-expanded={open} onClick={()=>setOpen(!open)}>القائمة</button><span className="topbar-note"><span className="avatar">{user.displayName.slice(0,1)}</span>{user.displayName} · {user.role==='ADMIN'?'مدير':'محرر'}</span>{control}</header><main id="main">{children}</main><footer>منصة إيران الآن · جميع الأوقات بتوقيت بيروت</footer></div></div>;
}
