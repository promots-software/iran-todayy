...
export function AutoPublishPanel({settings,heartbeat,controls=false,recovery=false}:{controls?:boolean;recovery?:boolean;settings:AppSettings|null;heartbeat:WorkerHeartbeat|null|undefined}){
 ...
 <p>الوضع المطلوب: {s.policy?.state==='ACTIVE'?'النشر التلقائي':'النشر اليدوي'} · حالة الناشر الفعلية:</p><p><strong>{s.enabled?'مفعّل — ENABLED':'معطّل — DISABLED'}</strong> · {reasons[s.reason]}</p>
 ...
 {controls&&<PublishingModeForm automatic={s.policy?.state==='ACTIVE'} canEnable={s.canEnable} canDisable={s.canDisable} expected={s.expected} requestId={randomUUID()}/>}
