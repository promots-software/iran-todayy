...
export function AutoPublishPanel({settings,heartbeat}:{settings:AppSettings|null;heartbeat:WorkerHeartbeat|null|undefined}){
 ...
 {(s.canDisable||s.canEnable)&&<OperationControl kind="AUTO_PUBLISH" target="1" value={s.canDisable?'false':'true'} expected={s.expected} requestId={randomUUID()} label={s.canDisable?'تعطيل النشر التلقائي':'تفعيل النشر التلقائي'}/>}
 {!s.canDisable&&!s.canEnable&&<p>التفعيل غير متاح حتى تتحقق صلاحية التسليم وجاهزية الناشر؛ لا يتجاوز هذا التحكم شروط الأمان.</p>}
