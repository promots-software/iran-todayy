...
export function HumanEditor({kind,id,revision,title,body,draftId,digest,status,locked,publicationImageId}:{kind:'post'|'news';id:string;revision:number;title:string;body:string;draftId?:string;digest?:string;status?:string;locked:boolean;publicationImageId?:string|null}){
 ...
 <label>النص العربي<textarea form={formId} name="body" value={editedBody} onChange={e=>setBody(e.target.value)} required maxLength={4096} rows={10}/></label>
 ...
 {draftId&&status==='DRAFT'&&<dialog ref={dialog}><h2>اعتماد الخبر</h2><p className="original">{title}{'\n\n'}{body}</p><form action={approve} className="form-panel">
