...
test('actual-request cost reserve and unchanged $1 ceiling; no maximum-call preclaim reserve',()=>{
 const now=Date.now();assert.equal(budgetDecision([],100,now,1024).reservedUsd,(100*.25+1024*1.5)/1e6);
 assert.equal(budgetDecision([{at:now,usd:.99}],100,now,1024).allowed,true);
 assert.equal(budgetDecision([{at:now,usd:1}],100,now,1024).reason,'PROVIDER_COST_WAIT');
