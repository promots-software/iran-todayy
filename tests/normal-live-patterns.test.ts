...
for(const source of ['المجلس في العاصمة قال الوزير: افتتحت مدرسة.','المجلس في العاصمة و الوزارة: افتتحت مدرسة.','المجلس في العاصمة والوزارة: افتتحت مدرسة.','ذكر الوزير المجلس في العاصمة: افتتحت مدرسة.'])test('qualified-heading matching cannot cross another speaker: '+source,()=>{assert.throws(()=>validateSpeakerEvidence(source,ev(source,source),ev(source,'المجلس')),/SPEAKER_ATTRIBUTION_MISMATCH/);});
test('later unique mention is not the speaker of an earlier statement; diagnostic retains exact field',async()=>{
 const source='أعلنت مؤسسة افتتاح المدرسة. وقالت إن الدوام يبدأ غداً. وأكدت الجهة ذلك.';const raw={relevance:'POLITICAL_NEWS',actors:[],action:null,object:null,location:null,event_time:null,statements:[{evidence:{excerpt:'وقالت إن الدوام يبدأ غداً.',context:source},speaker:{excerpt:'الجهة',context:source}}]};let calls=0;
 await assert.rejects(normalStage('extract',async repair=>{calls++;if(repair)assert.deepEqual(repair.issues,[{code:'SPEAKER_ATTRIBUTION_MISMATCH',path:['statements','f1','speaker']}]);return validateMinimalExtraction(raw,source);}),/AI_SCHEMA_REPAIR_FAILED/);assert.equal(calls,2);
});
for(const source of ['وسم تجريبي — افتتح المجلس مدرسة جديدة.','برچسب آزمایشی — شورا مدرسه جدیدی افتتاح کرد.','Experimental label — The council has opened a new school.'])test('live coverage failure reproduces with exact uncovered offsets: '+source,()=>{
 ...
 assert.throws(()=>validateNormalExtractionCoverage(source,x),(e:unknown)=>{assert(e instanceof ProcessingError);assert.equal(e.code,'DIRECT_MATERIAL_COVERAGE_FAILED');assert(e.diagnostic&&'issues'in e.diagnostic);assert.deepEqual(e.diagnostic.issues[0].path,['sourceUnits','u1','start',0,'end',source.indexOf(excerpt)]);return true;});
});
