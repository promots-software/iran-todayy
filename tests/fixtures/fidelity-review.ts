...
export function supportedLedger(source:string,publication:PublicationUnit[]){
 ...
 return {
  claims:publication.map(p=>({publicationId:p.id,excerpt:p.text,sourceUnitIds:content.map(u=>u.id),verdict:'SUPPORTED' as const,explanation:'Explicit offline fixture verdict; source meaning is supported.'})),
