...
test('database pagination: over 100 review/approval rows, pre-limit eligibility, null fallback, ties, holds and independent unresolved pages',{skip:!process.env.TEST_DATABASE_URL},async()=>{
 ...
 try{
  ...
  await db.sourcePost.createMany({data:Array.from({length:121},(_,i)=>({id:ids[i],sourceId:tag,sourcePostId:String(i),sourceUrl:'https://t.me/offline/'+i,originalContent:'خبر',sourcePublishedAt:at,ingestedAt:at,status:'NEEDS_REVIEW',error:'UNSUPPORTED_OUTPUT',metadata:{hasMedia:i===0,hasPhoto:i===0}}))});
  await db.newsEvidence.create({data:{newsItemId:ids[0],sourcePostId:ids[0]}});
  ...
  for(const mode of ['review','approval'] as const){const expected=allNews.filter(n=>{const ready=readEditorialState(n.validationResult,n.status,n.error)==='READY_TO_PUBLISH';return mode==='approval'?['PENDING_APPROVAL','APPROVED','NEEDS_REVIEW'].includes(n.status)&&!n.error&&['PASSED','NEEDS_REVIEW'].includes(n.validationStatus)&&(ready||n.status==='APPROVED'):n.status==='NEEDS_REVIEW'&&(!n.humanDraft||n.humanDraft.status==='DRAFT')&&!ready;}).map(n=>n.id);
