...
test('database pagination: over 100 review/approval rows, pre-limit eligibility, null fallback, ties, holds and independent unresolved pages',{skip:!process.env.TEST_DATABASE_URL},async()=>{
 ...
 try{
  ...
  for(const mode of ['review','approval'] as const){const expected=allNews.filter(n=>{const ready=readEditorialState(n.validationResult,n.status,n.error)==='READY_TO_PUBLISH';return mode==='approval'?!n.humanDraft&&['PENDING_APPROVAL','APPROVED','NEEDS_REVIEW'].includes(n.status)&&!n.error&&['PASSED','NEEDS_REVIEW'].includes(n.validationStatus)&&(ready||n.status==='APPROVED'):n.status==='NEEDS_REVIEW'&&!n.humanDraft&&!ready;}).map(n=>n.id);
