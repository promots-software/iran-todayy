...
test('identical comparison inputs need no provider answer without hiding multiple event ambiguity',async()=>{
 ...
 assert.equal(calls,0);assert.equal(result.classification,'UNCERTAIN_MATCH');assert.equal(result.candidates.length,2);
});
