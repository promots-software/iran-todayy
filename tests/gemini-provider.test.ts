...
test('Gemini atom selection uses fixed model, no thinking and leaves 503 retry to durable queue',async()=>{
 ...
 await assert.rejects(provider.draft({content:f.content,understanding:f.understanding,rules:ruleSet},new AbortController().signal),/GEMINI_HTTP_503/);assert.equal(calls,1);
});
