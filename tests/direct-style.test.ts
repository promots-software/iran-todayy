...
test('BOTH modes use Iran relevance without a second category/importance gate',()=>{
 ...
 assert.equal(selectionBlocksDraft(value,'DIRECT'),true);assert.equal(selectionBlocksDraft(value,'NORMAL'),true);
});
