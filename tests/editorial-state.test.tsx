...
test('editor sees distinct ready, review, technical and filtered Arabic states',()=>{
 ...
 assert.match(ready,/جاهز للنشر/);assert.match(ready,/قرار الإرسال مستقل/);assert.ok(!ready.includes("النشر التلقائي معطّل"));assert.ok(!ready.includes('يحتاج مراجعة'));
