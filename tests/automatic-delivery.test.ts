...
for(const mode of ['NORMAL','DIRECT'] as const)test(mode+' canary gates, human safety, concurrency, journal and restart',{skip:!process.env.TEST_DATABASE_URL},async()=>{
 ...
 const direct=new GeminiLanguageProvider('offline',async(_url,init)=>{const {coverage,publication,...matching}=raw;void coverage;return Response.json({candidates:[{finishReason:'STOP',content:{parts:[{text:JSON.stringify(JSON.parse(String(init?.body)).generationConfig.responseJsonSchema.properties.statements?matching:{title:publication.title.text,body:'',diagnostics:[]})}]}}],usageMetadata:{promptTokenCount:10,candidatesTokenCount:10}});});
