...
for(const mode of ['NORMAL','DIRECT'] as const)test(mode+' canary gates, human safety, concurrency, journal and restart',{skip:!process.env.TEST_DATABASE_URL},async()=>{
 ...
 const direct=new GeminiLanguageProvider('offline',async()=>Response.json({candidates:[{finishReason:'STOP',content:{parts:[{text:JSON.stringify(raw)}]}}],usageMetadata:{promptTokenCount:10,candidatesTokenCount:10}}));
